...
namespace Lykke.Service.HFT.Services
{
    public class MatchingEngineAdapter : IMatchingEngineAdapter
    {
            ...
            double price, bool cancelPreviousOrders = false)
        {
            ...
            var response = await _matchingEngineClient.PlaceLimitOrderAsync(requestId.ToString(), clientId, assetPairId, (OrderAction)orderAction, Math.Abs(volume), price, cancelPreviousOrders);
            if (response.Status == MeStatusCodes.Ok)
            {
